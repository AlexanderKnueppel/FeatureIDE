...
/**
 * Abstract Verification is the abstract super Class for all verification
 * methods.
 * 
 * @author marlen
 */
public abstract class AbstractVerification {
	...
	protected static Map<String, Map<String, Method>> methodMap = new HashMap<>();
	...
	public void warmUp(File location, String method, Map<String, Map<String, Method>> methodMap) {
		this.methodMap = methodMap;
		this.methodForVerification = method;
		if (Configuration.warmUp) {
			ProofHandler account = getAccountConstructor(location);
			if (account != null) {
				keyHandler.startMetaProductProof(account, location, DefaultStrategies.defaultSettingsForMetaproduct(),
						maxRuleApplication, null, null, methodMap);
				account.removeProof();
			}
		}
	}
	 ...
	 * @param stubReplay
	 */
	protected void performFeaturestubVerification(File projectDir, File evalPath, boolean firstVersion,
			boolean stubReplay) {
		...
		for (File f : featurestubs) {
			...
			saveFeatureStubPath = evalPath.getAbsolutePath() + FILE_SEPERATOR + FileManager.savedProofsDir
					+ FILE_SEPERATOR + currentFeatureStub;
			...
			for (ProofHandler proofHandler : proofHandlers) {
				...
				if (firstVersion || !proofAlreadyExists(proofHandler, evalPath, f.getParentFile())) {
					File oldPartialProof = getOldFeatureStubProof(proofHandler,
							FileManager.getProjectv1Path(projectDir));
					if (!firstVersion && stubReplay && oldPartialProof != null) {
						keyHandler.replayFeatureStubProof(proofHandler, oldPartialProof, saveFeatureStubPath,
								maxRuleApplication, DefaultStrategies.defaultSettingsForFeatureStub());
						proofHandler.saveProof(saveFeatureStubPath);
						proofHandler.setFeaturestub(f.getParentFile().getName());
					} else {
						try {
							keyHandler.startAbstractProof(proofHandler, maxRuleApplication,
									DefaultStrategies.defaultSettingsForFeatureStub());
							proofHandler.saveProof(saveFeatureStubPath);
							proofHandler.setFeaturestub(f.getParentFile().getName());
						} catch (Exception e) {
							e.printStackTrace();
	...
	protected void performMetaproductVerification(File projectDir, File evalPath) {
		...
		// parse alle Records to a map
		Map<String, Map<String, Record>> recordMap = new HashMap<>();
		FillMethodMap.parseRecordsInMetaproduct(projectDir, recordMap);

		for (ProofHandler proofHandler : proofHandlers) {
			try {
				// System.out.println("Hier nach sollte die methodMap erscheinen");
				// TestFillMethodMap.printMap(methodMap);
				File reuse = getFeatureStubProof(proofHandler, featurestubs);
				keyHandler.startMetaProductProof(proofHandler, reuse, DefaultStrategies.defaultSettingsForMetaproduct(),
						maxRuleApplication, metaproductPath, recordMap, methodMap);
				File file = proofHandler.saveProof(metaproductPath);
				KeyHandler.replaceJavaSource(file);
				...
				ProofInformation aTotal = new ProofInformation(proofHandler.getTypeName(), proofHandler.getTargetName(),
						proofHandler.getFeaturestub(), proofHandler.getStat(), proofHandler.getReusedStat(),
						proofHandler.isClosed());
				if (reusedProof != null) {
	...
	public void setProofList(List<ProofHandler> proofHandlers) {
		for (ProofHandler p : proofHandlers) {
			ProofInformation proofInformation = new ProofInformation(p.getTypeName(), p.getTargetName(),
					p.getFeaturestub(), p.getStat(), p.getReusedStat(), p.isClosed());
	...
	public boolean proofAlreadyExists(ProofHandler a, File evalPath, File featurestub) {
		File savedProofsPath = new File(
				evalPath + FILE_SEPERATOR + FileManager.savedProofsDir + FILE_SEPERATOR + featurestub.getName());
	...
	private static void removeDispatcher(List<ProofHandler> proofs) {
		...
		for (ProofHandler proofInformation : proofs) {
			for (ProofHandler proofInformation2 : proofs) {
				...
				if (proofInformation2.getTypeName().equals(proofInformation.getTypeName())
						&& proofInformation != proofInformation2) {
	...
	/**
	 * Returns the proof that should be reused in the second phase of verification
	 * for proof ap
	 * 
	 * @param proofHandler
	...
	protected static File getFeatureStubProof(ProofHandler proofHandler, List<File> featureStubProofs) {
		...
		if (biggestFile == null) {
			System.out.println("Biggestfile is null! Target name was: " + proofHandler.getTargetName()
					+ "; Type name was: " + proofHandler.getTypeName());
	...
	public static String getSaveName(ProofInformation proof, File projectDir) {
		...
		File featurestubFile = new File(projectDir.getAbsolutePath() + FILE_SEPERATOR + FileManager.featureStubDir
				+ FILE_SEPERATOR + proof.getTypeName() + ".java");
		File metaproduct = new File(projectDir.getAbsolutePath() + FILE_SEPERATOR + FileManager.metaproductDir
				+ FILE_SEPERATOR + proof.getTypeName() + ".java");
		if (MetaProductBuilder.checkForOriginal(featurestubFile, proof.getFeaturestub())
				|| MetaProductBuilder.checkForMethod(methodname + "_" + proof.getFeaturestub(), metaproduct)) {
	 ...
	 * @param savePath
	 */
	public void fullProofReuse(File evalPath, List<ProofHandler> proofList, StrategyProperties s, boolean firstVersion,
			String savePath, KeyHandler keyHandler) {
		Map<String, Map<String, Method>> methodMap = new HashMap<>();
		BuildMap.parseMethodsInMetaproduct(evalPath, methodMap);
		if (firstVersion) {
			for (ProofHandler aproof : proofList) {
				keyHandler.startMetaProductProof(aproof, null, s, maxRuleApplication, savePath, null, methodMap);
				aproof.saveProof(savePath);
			}
		} else {
			for (ProofHandler aproof : proofList) {
				keyHandler.startMetaProductProof(aproof, reuseFullProof(evalPath, aproof), s, maxRuleApplication,
						savePath, null, methodMap);
				aproof.saveProof(savePath);
			}
		}
	...
	/**
	 * Returns the ProofHandler Object from Account Only for BankAccount Evaluation
	 * 
	 * @param location
	 * @return proofHandler
	 */
	public ProofHandler getAccountConstructor(File location) {
		...
		for (ProofHandler proofHandler : proofsHandlers) {
			if (proofHandler.getTypeName().contains("Account") && proofHandler.getTargetName().contains("Account")) {
				return proofHandler;
			}
		}
		return null;
