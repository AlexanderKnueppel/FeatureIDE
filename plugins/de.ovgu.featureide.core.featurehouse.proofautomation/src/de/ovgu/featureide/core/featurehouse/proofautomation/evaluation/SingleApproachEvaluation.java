...
public class SingleApproachEvaluation extends Evaluation {
	...
	/**
	 * Constructor for execution in an verification approach Uses a given
	 * evaluatePath instead of a new
	 * 
	 * @param f
	 * @param evalVersion
	 * @param evaluatePath
	...
	public SingleApproachEvaluation(File f, int evalVersion, String evaluatePath, String method) {
		...
		this.statistics = new File(
				evaluatePath + FILE_SEPERATOR + "Evaluation Results-VA" + this.evalVersion + ".xlsx");
	}
	 ...
	 * 
	 * @see
	 * de.ovgu.featureide.core.featurehouse.proofautomation.evaluation.Evaluation#
	 * performEvaluation()
	 */
	@Override
	public void performEvaluation() {
		...
		if (evalVersion == 1) {
			...
			File transactionAccount = new File(toEvaluate.getAbsolutePath() + FILE_SEPERATOR
					+ FileManager.featureStubDir + FILE_SEPERATOR + "Transaction" + FILE_SEPERATOR + "Account.java");
			File lockAccount = new File(toEvaluate.getAbsolutePath() + FILE_SEPERATOR + FileManager.featureStubDir
					+ FILE_SEPERATOR + "Lock" + FILE_SEPERATOR + "Account.java");
			FeatureStubBuilder.prepareForVerification(transactionAccount, lockAccount);

			// copy
			FileManager.copyCompleteFolderContent(fstubPath,
					new File(evaluatePath.getAbsolutePath() + FILE_SEPERATOR + FileManager.featureStubDir));

		}

		File metaproductPath = new File(toEvaluate.getAbsolutePath() + FILE_SEPERATOR + FileManager.metaproductDir);
		FileManager.copyCompleteFolderContent(metaproductPath,
				new File(evaluatePath.getAbsolutePath() + FILE_SEPERATOR + FileManager.metaproductDir));
		AbstractVerification abstractVerification = null;
		switch (evalVersion) {
		case 1:
			abstractVerification = FefalutionFamilyProofReplay.getInstance();
			break;
		case 2:
			abstractVerification = Metaproduct.getInstance();
			break;
		case 3:
			abstractVerification = ConcreteContracts.getInstance();
			break;
		case 4:
			abstractVerification = MethodInling.getInstance();
			break;
		case 5:
			abstractVerification = ThuemEtAl.getInstance();
			break;
		case 6:
			abstractVerification = ThuemEtAlReuse.getInstance();
			break;
		}
